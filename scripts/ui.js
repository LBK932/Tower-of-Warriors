...
ui.action.build = function (type) {
  ...
  var turret = {
    ...
    img: document.querySelector(
      "#control-turrets [data-name=" + type + "] img"
    ),
...
ui.action.upgrade = function (stat) {
  ...
  if (game.selection.status === "selected" && cost && game.cash - cost >= 0) {
    ...
    levels.full =
      levels.damage === 10 && levels.rate === 10 && levels.range === 10;
    turret.cost += cost;
    game.cash -= cost;
    game.spent += cost;
    ui.action.refresh();
...
ui.action.refresh = function () {
  ...
  if (game.selection) {
    ...
    ["Damage", "Rate", "Range"].forEach(function (proper) {
      ...
      $("control-manage-" + id).innerHTML =
        proper + " (" + level + ")<br>" + (cost && "$" + cost);
    });

    $("control-manage-sell").innerHTML =
      "Sell<br>$" + Math.round(turret.cost * 0.7);

    $("control-manage-stats").innerHTML =
      turret.kills +
      " kills<br>" +
      ((turret.kills / game.kills || 0) * 100).toFixed(2) +
      "% of &sum;";
  }
};
...
$("pages-canvas").addEventListener(
  ...
  function (evt) {
    ...
    var tile =
      game.tiles[
        Math.ceil((evt.pageX - this.offsetLeft) / 5) +
          "," +
          Math.ceil((evt.pageY - this.offsetTop) / 5)
      ];
...
$("control-timer").addEventListener(
  "click",
  function (evt) {
    if (!game.paused) {
      game._wave = game.ticks - 1200;
...
$("control-fast").addEventListener(
  "click",
  function (evt) {
    if (!game.paused) {
      this.style.backgroundColor = (game.fast = !game.fast)
        ? "#97D164"
        : "#85ADE6";
      game.pause();
      game.start();
...
$("control-pause").addEventListener(
  "click",
  function (evt) {
    ...
    .forEach(function (cur, i, a) {
      ...
      if (Math.abs(dx) > Math.abs(dy)) {
        ...
        while (cur.x !== next.x) {
          ...
          for (var i = -3; i <= 4; i++) {
            game.tiles[
              Math.round(cur.x / 5) + "," + (Math.round(m * cur.x + b) / 5 + i)
            ] = true;
          }
        }
      } else if (dy !== 0) {
        ...
        while (cur.y !== next.y) {
          ...
          for (var i = -3; i <= 4; i++) {
            game.tiles[
              Math.round(m * cur.y + b) / 5 + i + "," + Math.round(cur.y / 5)
            ] = true;
          }
        }
      }
...
ui.handletweets = function (data) {
  ...
  data.results.forEach(function (tweet) {
    var m = tweet.text.match(
      /I scored (\d+) \((\d+) kills, \$(\d+) spent\) on (Loopy|Backtrack|Dash) in #canvastd/i
    );
